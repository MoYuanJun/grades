...
$uploadCatalog = $_POST['uploadCatalog'];
$upload = new UploadFile(true, "../img/{$uploadCatalog}/", array('jpg', 'jpeg', 'png')); //创建文件上传类
$upload->upload_file($file);//调用文件上传类并执行方法，导入files

if( $upload->get_msg() === '文件上传成功' ){ //获取文件上传信息（失败 || 成功）
    $result['error'] = '1';
    $result['img'] = $com_img_default_url."/{$uploadCatalog}/".$upload->get_new_name();  //获取上传后的文件名
} else {
    $result['error'] = '1';
}
