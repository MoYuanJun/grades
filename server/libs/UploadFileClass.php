...
/**文件上传类
 * =================基本使用方法：===================
 * require_once '../libs/UploadFileClass.php';  //导入类：
 * $file = $_FILES['file'];  //获取通过接口上传的数据
 * $upload = new UploadFile(true, '../img/commodity/', array('jpg', 'jpeg', 'png')); //通过构造函数创建类
 * $upload->upload_file($file);  //执行类方法，导入数据进行文件上传
 * echo $upload->get_msg();        //获取信息（上传成功？失败？）
 * echo $upload->get_new_name();  //获取上传后的新的文件名
 */
class UploadFile{
